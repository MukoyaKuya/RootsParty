...
class Command(BaseCommand):
    help = "Update the 'Move Capital to Isiolo' manifesto item and its evidence."
    ...
    """
        item.order = 8
        item.save()
        self.stdout.write(self.style.SUCCESS(f"Updated Manifesto Item: {item.title}"))
        ...
        for proof in evidence_list:
            ...
            self.stdout.write(f"Added evidence for {proof['country']}")
